...
First exploring fitting some variables with polynomials
```{r}
fit <- lm(prostate$Cscore~poly(prostate$lpsa, 4), data=prostate)
summary(fit)
plot(prostate$lpsa, prostate$Cscore)
lines(smooth.spline(prostate$lpsa, predict(fit)), col='blue', lwd=2)
